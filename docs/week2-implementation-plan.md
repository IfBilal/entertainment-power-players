...
1. **Sign in with Apple: deferred, not dropped, not scheduled to any week.** No Apple Developer account exists yet — **the client will provide one**, and Apple sign-in gets built once it does, whenever that is. It is not assigned to Week 3, Week 4, or any other week right now — it has no place on the calendar until the account actually arrives, at which point it gets slotted into whatever week is current at the time. `SignUpScreen`'s Apple button gets removed or visibly disabled/"coming soon" for now (decide during Workstream A) so "Auth complete" for Week 2 means **email/password + Google** only.
...
Per handbook §8 ("No new dependency without approval") — new mobile dependency this week: `@react-native-google-signin/google-signin`. Flagged and approved via the above.
...
- **Build discipline (confirmed with user, free EAS tier only — 15 builds/month/platform, no paid plan)**: everything except Google sign-in itself keeps developing on plain Expo Go (QR-scan, instant reload, zero build cost) — Directory, email/password auth, favourites, admin panel, all of it. Only Google sign-in needs the dev-client build, and only rebuilt if something about it changes.
- **Keep Google sign-in lazy, not guarded-hidden**: the button stays visible in plain Expo Go too — simpler than hiding it. The requirement is that nothing from `@react-native-google-signin/google-signin` runs *eagerly* (no `GoogleSignin.configure()` at app startup/module load) — all of its calls (configure + sign-in) happen lazily, only inside the button's own `onPress`, wrapped in try/catch. In the dev-client build it works normally; in plain Expo Go the native module isn't linked, the catch fires, and we show a friendly "Google sign-in isn't available in this preview — use the full build" message instead of a crash. Everything else in the app is unaffected either way, since nothing else touches this module.
- **Step 0 — EAS dev client build** (do once, only to actually test Google sign-in works): `eas build --profile development` per decision #4. Everything else in this week's work never needs it.
...
- **Manual device verification** (I cannot do this myself): **everything except Google sign-in verifies on plain Expo Go as usual** (sign up with email/password, browse a real category, favourite/unfavourite, mark a contact as contacted, log out, log back in and confirm state persisted; log into the admin panel, add a contact, confirm it appears in the app without a restart). **Google sign-in specifically requires the EAS dev client build (Workstream A, Step 0)** — test that one thing there, don't rebuild for anything else.
