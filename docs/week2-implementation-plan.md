...
- **Guard Google sign-in behind an Expo Go check**: detect `Constants.appOwnership === 'expo'` (or the SDK 53+ equivalent) at the top of wherever `@react-native-google-signin/google-signin` is imported/used, and hide or disable the Google button with a "not available in this preview" message when running in plain Expo Go, instead of crashing. This is what makes the "stay on Expo Go for everything else" plan actually work — without the guard, having the native module in the bundle at all can break Expo Go for *every* screen, not just the Google button.
